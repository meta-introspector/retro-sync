...
const ConnectWallet = () => {
  const { wallet, isConnecting, error, connectTronLink, connectWalletConnect, disconnect, shortenAddress, setError } = useWallet();
  const [menuOpen, setMenuOpen] = useState(false);
  const [showOnboarding, setShowOnboarding] = useState(false);
  const [selectedChain, setSelectedChain] = useState<ChainId>("bttc");

  const handleConnect = async (type: "tronlink" | "walletconnect") => {
    setMenuOpen(false);
    if (type === "tronlink") {
